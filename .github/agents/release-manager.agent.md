...
5. Run `npm run prerelease` and confirm it is green before proposing the PR. This gate runs `npm ci --dry-run` (verifies lockfile sync), then builds the editor and the viewer library.
...
> Note: this repo's `npm run prerelease` intentionally excludes `npm run lint` (there is no committed ESLint config yet) and `npm test` (no test framework is configured). It mirrors the `npm ci` + build path that `publish.yml` uses.
